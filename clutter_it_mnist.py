...
### clutter_it_mnist.clutter_it(mnist.train.images[0:1,:], mnist, 1)
...
def clutter_it(imgd, mnist, flag_c, c_max=20, flag_d=1):
    ...
    for j in range(np.shape(imgd)[0]):
        ...
        if flag_d == 2:
            ...
            img1[50 - np.int(np.ceil(dum0_1)):50 + np.int(np.floor(dum0_1)),
            50 - np.int(np.ceil(dum0_1)):50 + np.int(np.floor(dum0_1))] = dum0_0  # place image centrally
        if flag_d == 1:
        ...
        if (dum1 > 0):
            for i in range(dum1):  # 8x8 patches being added
                ...
                img1[x + np.where(dum_img2)[0], y + np.where(dum_img2)[1]] = dum_img2[
                    np.where(dum_img2)[0], np.where(dum_img2)[1]]
        imgd_h[j, :] = np.reshape(img1, [1, 100 * 100])
    return imgd_h.astype(np.float32)
